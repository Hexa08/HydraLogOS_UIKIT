...
confirm_installation() {
    echo -e "${YELLOW}This will install HydraUI  UI components:${NC}
  • Layan GTK Theme
  • Candy Icon Pack
  • Custom Wallpaper
...
check_requirements() {
    ...
    for cmd in wget git make gsettings; do command -v $cmd >/dev/null 2>&1 || log ERROR "$cmd missing" && exit 1; done
...
install_theme() {
    ...
    sudo "$TEMP_DIR/Layan/install.sh" -d "$THEMES_DIR" >/dev/null 2>&1
...
install_icons() {
    ...
    sudo mv "$TEMP_DIR/candy" "$ICONS_DIR/Candy"
    spinner "Installing icons"
}
...
install_extensions() {
    ...
    (cd "$TEMP_DIR/blur" && make install >/dev/null 2>&1) &
    spinner "Installing blur-my-shell"
    
    git clone --depth 1 https://github.com/justperfection/just-perfection.git "$TEMP_DIR/just" >/dev/null 2>&1
    (cd "$TEMP_DIR/just" && make install >/dev/null 2>&1) &
    spinner "Installing just-perfection"
}
...
main() {
    mkdir -p "$TEMP_DIR"
    show_banner
    [[ -f "$INSTALL_MARKER" ]] && log WARN "HydraUI already installed"
    confirm_installation
    check_requirements
    install_dependencies
    install_theme
    ...
    configure_conky
    apply_settings
    log SUCCESS "HydraUI Pixel UI installation completed!"
}
